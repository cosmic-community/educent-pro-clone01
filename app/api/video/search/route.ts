...
async function searchEducationalVideo(topic: string, language: string): Promise<string> {
    ...
    const videoId = educationalVideos[topic] || 
                    Object.keys(educationalVideos).find(key => 
                        topic.toLowerCase().includes(key.toLowerCase())
                    ) || 
                    educationalVideos.default
    
    return `https://www.youtube.com/embed/${educationalVideos[videoId] || videoId}`
}